...
function my_theme_options_init() {
	...
	add_settings_field( 'featured_text', __( 'Home Page Slider :', 'language' ), 'my_settings_field_featured_text_input', 	'theme_options', 'general' );
...
if ( ! function_exists ( 'my_get_theme_options' ) ) {
	...
	'currency_text'	=> __('VND','language'),
	'price_text'     => __('Giá bán ','language'),
	'status_tag_text'     => __('Condition','language'),
	'top_deal_text'     => __('Top Deal','language'),
	'featured_text'     => __('Slider trang chủ','language'),
	...
	'interior_text'     => __('Màu nội thất','language'),
	'exterior_text'     => __('Màu ngoại thất','language'),
	...
	'miles_text'     	=> __('Lượt mua','language'),
	...
	'transmission_text'     	=> __('Hộp số','language'),
...
function my_settings_field_citympg_text_input() {

	$options = my_get_theme_options();

	?>

	<input type="text" name="my_theme_options[citympg_text]" id="citympg-text-input" value="<?php echo esc_attr( $options['citympg_text'] ); ?>" />

	<?php

}

function my_settings_field_highwaympg_text_input() {

	$options = my_get_theme_options();

	?>

	<input type="text" name="my_theme_options[highwaympg_text]" id="highwaympg-text-input" value="<?php echo esc_attr( $options['highwaympg_text'] ); ?>" />
...
<input type="text" name="my_theme_options[vehicle_type_text]" id="vehicle_type-text-input" value="<?php echo esc_attr( $options['vehicle_type_text'] ); ?>" /><label for="checkbox">
	...
	<li class='lialt'><strong><?php _e('Option 9:','language');?></strong>&nbsp;&nbsp;<input type="text" name="my_theme_options[vehicle_type_9]" id="vehicle_type-text-input" value="<?php echo esc_attr( $options['vehicle_type_9'] ); ?>" /></li>	
	<?php
}
...
function my_theme_options_validate( $input ) {
	...
	$output['vin_text'] = wp_filter_nohtml_kses( $input['vin_text'] );
	 if ( isset( $input['citympg_text'] ) && ! empty( $input['citympg_text'] ) )

	$output['citympg_text'] = wp_filter_nohtml_kses( $input['citympg_text'] );

	if ( isset( $input['highwaympg_text'] ) && ! empty( $input['highwaympg_text'] ) )

	$output['highwaympg_text'] = wp_filter_nohtml_kses( $input['highwaympg_text'] );
...
function custom_title_text( $title ){
		...
		$title = __('Enter Listing Title','language');
		}
		return $title;
}
...
//    remove_theme_mods();
set_post_thumbnail_size( 904, 446, true );
