...
function gtcd_custom_columns($column) {
		...
		switch ($column) {
			...
			<?php
				...
				if ( $fields['miles']){ echo '<div style="font-weight:bold;color:#454d51;margin-bottom:5px;">'.number_format($fields['miles']).' '.$options['miles_text'].'</div>';} elseif ($fields['miles'] == '0' ){ echo _e('0','language').' '.$options['miles_text'];} else {echo '';} 			
				...
				if(isset($fields2['VIN'])) {  echo '<div style="color:#454d51;"><span style="font-weight:bold">'.$options['vin_text'].'</span>: '.$fields2['VIN'].'</div>';  } 
				else  {  }
