...
function sidebars_and_widgets()
{
	...
	$sidebars = array(		
		...
		'search'  => array(
							'id'            => 'search',
							'name'          => __( 'AutoMax Sidebar', 'language' ),
							'description'   => __( 'Add all AutoMax widgets here', 'language' ),
		...
		'carousel' => array(
							'id'            => 'carousel',
							'name'          => __( 'AutoMax Carousel', 'language' ),
		...
		'welcome' => array(
							'id'            => 'welcome',
							'name'          => __( 'Welcome Legend', 'language' ),
							'description'   => __( 'Add a welcome message on home page.', 'language' ),
				),
		'footer' => array(
							'id'            => 'footer',
	...
	$active_widgets['welcome'][0] = 'welcome_widget-' . $counter;
	$welcome_title[ $counter ] = array ( 'title' => 'Welcome' );
	update_option( 'widget_welcome_widget', $welcome_title );
...
class Loan_Calculator extends WP_Widget {
	public function __construct() {
		parent::__construct(
			'create_calc',
			__( 'AutoMax: Loan Calculator', 'language' ),
			array(
...
public	function __construct() {                    
			parent::__construct( 'carfax_widget', 'AutoMax: Carfax Widget');
	}
 ...
 if (!empty( $fields['VIN'])){ ?><div><a class="carfax" target="_blank" href='http://www.carfax.com/cfm/ccc_DisplayHistoryRpt.cfm?partner=WDB_0&vin=
<?php echo $fields['VIN'];?>'><img src='<?php echo get_bloginfo('template_url'); ?>/assets/images/common/carfax.png' width='237' height='105' border='0'></a>
 ...
 if (isset( $fields['vin'])){ ?><div><a class="carfax" target="_blank" href='http://www.carfax.com/cfm/ccc_DisplayHistoryRpt.cfm?partner=WDB_0&vin=
<?php echo $fields['vin'];?>'><img src='<?php echo get_bloginfo('template_url'); ?>/assets/images/common/carfax.png' width='237' height='105' border='0'></a>
</div><?php  } else {   echo '';  }
...
class Footer3_Widget extends WP_Widget {
	...
	{                      // id_base        ,  visible name
		parent::__construct( 'footer_3', 'AutoMax: Footer 3 Module Widget' );
	}
	function widget($args, $instance) {
		extract($args, EXTR_SKIP);
		$title = empty($instance['title']) ? ' ' : apply_filters('widget_title', $instance['title']);
...
class Footer4_Widget extends WP_Widget {
	public function __construct()
	{                      // id_base        ,  visible name
		parent::__construct( 'footer_4', 'AutoMax: Footer 4 Module Widget' );
	}
	function widget($args, $instance) {
		extract($args, EXTR_SKIP);	
		$title = empty($instance['title']) ? ' ' : apply_filters('widget_title', $instance['title']);
...
class Footer2_Widget extends WP_Widget {		
	public function __construct()
	{                      // id_base        ,  visible name
		parent::__construct( 'footer_2', 'AutoMax: Footer 2 Module Widget' );
	}
	function widget($args, $instance) {
		extract($args, EXTR_SKIP);
		$title = empty($instance['title']) ? ' ' : apply_filters('widget_title', $instance['title']);?>
...
class Footer1_Widget extends WP_Widget {
	public function __construct()
	{                      // id_base        ,  visible name
		parent::__construct( 'footer_1', 'AutoMax: Footer 1 Module Widget' );
	}
	public function widget($args, $instance) {
		extract($args, EXTR_SKIP);	
		$title = empty($instance['title']) ? ' ' : apply_filters('widget_title', $instance['title']);
...
function __construct() {
		parent::__construct(
			'feat_widget', 
			__('AutoMax: Featured Vehicles Module', 'language'), 
...
{
	public	function __construct() {
			parent::__construct( 'phone_widget', 'AutoMax Header Phone Number' );
	}
	function widget($args, $instance) {
			extract($args, EXTR_SKIP);
			$title = empty($instance['title']) ? ' ' : apply_filters('widget_title', $instance['title']);
...
{
	public	function __construct(){
			parent::__construct( 'gtcd_widget', 'AutoMax Search Module' );
	}
	function widget($args, $instance) {
			extract($args, EXTR_SKIP);	
			$title = empty($instance['title']) ? ' ' : apply_filters('widget_title', $instance['title']);	
...
class Carousel extends WP_Widget
{
	public function __construct()
	{                      // id_base        ,  visible name
		parent::__construct( 'create_carousel', 'Automax: Carousel' );
	}
	function widget($args, $instance) {		
...
?> <a href="<?php the_permalink();?>"><?php gorilla_img ($post->ID,'large'); ?></a>
				...
				?><?php echo $post->post_title;?></span> 
								 ...
								 <?php 
} else { ?><a href="<?php the_permalink();?>"><?php the_post_thumbnail('large', array( 'class'	=> "img-responsive"));} ?></a>
...
class Full_Specs extends WP_Widget{
		public function __construct()
	{                    
		parent::__construct( 'full_specs', 'AutoMax Vehicle Specifications' );
	}
		function widget($args, $instance) {
		$title = empty($instance['title']) ? ' ' : apply_filters('widget_title', $instance['title']);
...
class Welcome_Widget extends WP_Widget {
	...
	public	function __construct() {                    
			parent::__construct( 'welcome', 'AutoMax: Welcome Paragraph');
	}
	function widget($args, $instance) {
			extract($args, EXTR_SKIP);
			$title = empty($instance['title']) ? ' ' : apply_filters('widget_title', $instance['title']);
			$text  = apply_filters( 'widget_textarea', empty( $instance['text'] ) ? '' : $instance['text'], $instance );
			echo '<div class="hideOnSearch col-sm-12">';
			echo '<div class="welcome_widget">';
			echo '<h1>'.$title.'</h1>';
			echo $after_title;
			echo wpautop( $text ); 
			echo '</div>';
...
<p>
  <label for="<?php echo $this->get_field_id('title'); ?>">
    <?php _e("Enter Welcome title","language");?>
    <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
  </label>
</p>

<textarea id="<?php echo $this->get_field_id('text'); ?>" name="<?php echo $this->get_field_name('text'); ?>" rows="10" cols="33" ><?php echo wpautop( $text ); ?></textarea><?php
}
}
